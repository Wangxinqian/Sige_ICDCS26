...
async def main():
    ...
    await party.send({"kind": "mask", "from": me, "r": p_0}, to=peer_idx)
    ...
    await party.send({"kind": "mask2", "from": me, "r": p_0}, to=peer_idx)
    _, _, peer_mask_msg = await recv_matching_with_pending(
    ...
    await party.send({"kind": "mask3", "from": me, "r": m_ij_0_list}, to=peer_idx)
    _, _, peer_mask_msg = await recv_matching_with_pending(
            party, want_kind="mask3", want_from=peer_name, pending=pending, recv_timeout=30.0,)
    m_ij_1_list = peer_mask_msg['r']
